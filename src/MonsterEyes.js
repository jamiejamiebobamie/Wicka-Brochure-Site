...
class MonsterEyes extends Component{
    constructor(props){
        super(props)
        this.state = {
            animation: 'openEyes 0s steps(30) reverse',
            open: false,
            random: 0,
            blinking:false
        }
    ...
    randomBlink(){
        if (Math.random() > .5 && this.state.blinking === false){
            this.setState( {
                animation: 'openEyes 1.2s steps(30) infinite alternate',
                blinking: true} )
            setTimeout( () => {
                this.setState(
                { animation: 'eyesOpened 1s steps(1) infinite alternate-reverse',
                  blinking: false})
            },2400);
        }
    }
    ...
    openEyes(){
        if (!this.state.open) {
            this.setState( {
            animation: 'openEyes 1.2s steps(30) reverse',
            open:true
            } )
            setTimeout( ()=> {
                this.setState( {
